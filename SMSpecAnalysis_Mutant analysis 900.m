...
skipfiles=[49 207 268 330 513 660 685];    %numbers don't need to be in any particular order; matrix can also be empty
...
dbl2norm=[44 155 167 219 441 451 540 541 546 749 754 773 801 822];   %force identified double-banded complex to be single-banded
readdir='D:\Sheela\Mutant 3 5C POC flushed e-5';
writedir='D:\Sheela\Mutant 3 5C POC flushed e-5\Analysis 20130416';

% Background subtraction
bgspec=true;           %true if a file containing a bg spectrum exists. Useful if background intensity < 0. Otherwise background subtraction is done automatically.
...
subtrbl=true;          %true if a baseline should be subtracted from all spectra
subtrpeak=true;         %true if the background sometimes contains a real spectrum

% Intensity and SNR thresholds
CRthr=700;    %threshold of individual intensity values. Larger values are definitely due to cosmic rays.
Qthr=4;       %max SNR of Q spectra. If SNR is below this threshold, chances are good for a misfit of single-band spectra. Can be as small as 1.5 - 2, since Q states are not ocnsidered yet.
SNRdead=4;    %max SNR when complex is considered "dead" (if large SNR, set ~3.5, otherwise ~2.5)
SNRbulk=200;     %min SNR of clearly >1 complex
...
finddbls=true;  %checks for double bands. If false, then only necessary parameters need to be defined.
SNRdbl=7;           %SNR above which the script considers a double band
fwhmjump=5;         %jump in fwhm to consider double bands (8 is approx max for Lhcbs)
fwhmdouble=21;      %max fwhm of a single peak before the script considers double bands (~25 for Lhcbs, 26 for minors).
rmsthr=30;
findfarreds=true;   %typically small bands
farredminwav=750;   %minimum wavelength of far-red bands
intfarred=700;      %minimum intensity of a far-red band.
...
fwhm1=18; fwhm2=18;
skewness=0.03;
peak1=678; peak2=690; %typical peak positions for double-band spectra;
%specialpeakmin=679; specialpeakmax=681; %special category of peaks - not incorporated yet

% Boundary conditions for fitting
xmin=627; xmax=840; % maximum wavelength window considered for all calculations. Set e.g. to 0 and 1000, resp., if full range is required.
peakmin=670; peakmax=820; %note if < peakmin, complex prob underwent spectral bluing
peak2min=681; % lower boundary of red peak. Set to 0 if it should be determined automatically
fwhmmin=13; fwhmmax1=fwhmdouble; fwhmmax2=50; % 1 and 2 refer to blue and red peaks, respectively. Better to choose smaller max values and scale with wavelength
skewmin1=-.5; skewmin2=-.5; skewmax1=skewred; skewmax2=skewred;
...
%                                                specfit1=skewgaussfit4([wav,data],[fwhm1,peak1,skewness],fittype,lbs,ub);
    ...
    %                allskew2(badspecs)=0;
                    ...
                    for k=1:m
                        ...
                        afFigurePosition = [2 2 45 25]; % [pos_x pos_y width_x width_y]
