...
% Input files
allfiles=1:750; %A:B or matrix of individual spec files
...
readdir='D:\data\Werk\VU\data\FCP\Copy of FCPb Tjaart\spectra\pH65';
writedir='D:\data\Werk\VU\data\FCP\Copy of FCPb Tjaart\spectra\pH65\Analysis3';
...
CRthr=500;    %threshold of individual intensity values. Larger values are definitely due to cosmic rays.
Qthr=2.5;       %max SNR of Q spectra. If SNR is below this threshold, chances are good for a misfit of single-band spectra. Can be as small as 1.5 - 2, since Q states are not ocnsidered yet.
SNRdead=3;    %max SNR when complex is considered "dead" (if large SNR, set ~3.5, otherwise ~2.5)
SNRbulk=35;     %min SNR of clearly >1 complex
...
fwhmjump=9;         %jump in fwhm to consider double bands (8 is approx max for Lhcbs)
fwhmdouble=26;      %max fwhm of a single peak before the script considers double bands (~25 for Lhcbs, 26 for minors).
rmsthr=7;
...
fwhm1=20; fwhm2=25;
skewness=0.03;
peak1=676; peak2=710; %typical peak positions for double-band spectra;
...
fwhmmin=13; fwhmmax1=fwhmdouble; fwhmmax2=40; % 1 and 2 refer to blue and red peaks, respectively. Better to choose smaller max values and scale with wavelength
...
maxspecs = 120;  % max number of spectra per complex
