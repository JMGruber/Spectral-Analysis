% This script resolves the spectra from a user-given set of spectral sequences
% This is a BETA version, so please supply suggestions for improvements / changes
...
% Code written on Matlab R2011b. Other versions haven't been tested.
%
...
% Input files
allfiles=1:20;   %A:B or matrix of individual spec files
...
readdir='D:\Supercomplexes\091012 pH8 300nW 5 degC goc FLUSHED';
writedir='D:\Supercomplexes\091012 pH8 300nW 5 degC goc FLUSHED\ana';

% Background subtraction
bgspec=false;           %true if a file containing a bg spectrum exists. Useful if background intensity < 0. Otherwise background subtraction is done automatically.
bgfilename='bg.txt';    %is used when bgspec=true. File should be in readdir.
...
subtrbl=false;          %true if a baseline should be subtracted from all spectra

% Intensity and SNR thresholds
CRthr=500;    %threshold of individual intensity values. Larger values are definitely due to cosmic rays. pixel intensity
Qthr=1.5;       %max SNR of Q spectra. If SNR is below this threshold, chances are good for a misfit of single-band spectra. Can be as small as 1.5 - 2, since Q states are not ocnsidered yet.
SNRdead=2.5;    %max SNR when complex is considered "dead"
SNRbulk=25;     %min SNR of clearly >1 complex
...
finddblpeaks=true;  %checks for double bands. If false, then only necessary parameters need to be defined.
SNRdbl=3;           %SNR above which the script considers a double band
fwhmjump=8;         %jump in fwhm to consider double bands (8 is approx max for Lhcbs) relative change in fwhm
fwhmdouble=25;      %max fwhm of a single peak before the script considers double bands (~25 for Lhcbs). absolute
findfarreds=true;   %typically small bands
farredminwav=750;   %minimum wavelength of far-red bands
intfarred=250;      %minimum intensity of a far-red band. (Only necessary for case of negative baseline) integral value from 750 nm to the end
%skewblue=-0.1; %max skewness of a single peak before the script considers double bands to the blue - not incorporated yet
skewred=1.1; %same as skewblue, but for red tails
...
scalef_int=20;       %if scalewidth_int = true, fwhmdouble = fwhmdouble+(scalef_int/SNR)^2

%vibrational band
fitvib=false;            %true if an additional small red spectrum should be fitted
peakvib=740; fwhmvib=40; %typical peak position and fwhm of vib band
peakvibmin=720; peakvibmax=750; 
fwhmvibmax=70;
minvib=700;             %lower boundary of vibrational band (nm)
mainvsvibfactor=3;      %minimum ratio of main (blue) peak vs. vibrational band, before the latter is likely a real red band

% first estimations for fitting. If finddblpeaks=false, values for fwhm2 and peak2 will be ignored
fwhm1=21; fwhm2=30;
skewness=0.03;
peak1=678; peak2=720; %typical peak positions for double-band spectra;
%specialpeakmin=679; specialpeakmax=681; %special category of peaks - not incorporated yet

% Boundary conditions for fitting
xmin=600; xmax=840; % maximum wavelength window considered for all calculations. Set e.g. to 0 and 1000, resp., if full range is required.
peakmin=670; peakmax=820; %note if < peakmin, complex prob underwent spectral bluing
peak2min=710; % lower boundary of red peak. Set to 0 if it should be determined automatically
fwhmmin=17; fwhmmax1=fwhmdouble; fwhmmax2=40; % 1 and 2 refer to blue and red peaks, respectively. Better to choose smaller max values and scale with wavelength
skewmin1=-.8; skewmin2=-.5; skewmax1=skewred; skewmax2=skewred;

% Binning: input data
wavbin=2;   % wavelength binning: number of adjacent datapoints
wavbinsmall=3; % binning for small peaks (see SNRsmall)
...
maxspecs = 60;  % max number of spectra per complex
        ...
        if (isempty(lastgood)) 
          ...
          %%% 1. Identify blue or broad spectra by fitting average of all good (unQ) spectra %%%
                if size(unQspecs,2)>1
                    avgdata=mean(unQspecs,2); %first index being the wavelength
                else
                    avgdata=unQspecs;
                end
...
%                            end                            
                            if (specfit(1)<SQthr)||(specfit(3)<lbs(3)+.1)||(specfit(3)>ubs(3))  %SNR too small to resolve spectrum
                               specfit=[0 0 0 0];
                            elseif (specfit(3)>peak1+fwhm1/2)   %if clearly double, don't fit vib band to save time
                                if (specfit(4)<lbs(4)+.1) || (specfit(4)>ubs(4)-.1) || (specfit(3)<peak1+fwhm1/2) || (specfit(3)>ubs(3)-.1) || (specfit(2)>fwhmmax1+fwhmmax2)                           
                                    ...
                                    if (specfit(4)<lbs(4)+.1) || (specfit(4)>ubs(4)-.1) || (specfit(3)<max(wav(border),peak1+fwhm1/2)) || (specfit(3)>ubs(3)-.1)
                                         ...
                                         specfit(4)=0;
                                         if (specfit(2)>max(peakmx-peak1,2*fwhmmax1+fwhmmax2))&&(specfit(3)<ubs(3))&&(specfit(3)>lbs(3))
                                             broad=true;
                                             broads(i)=complex;
                                             break;
                                         elseif (specfit(2)<lbs(2)+.1)||(specfit(2)>ubs(2)-.1)||(specfit(3)<max(wav(border),lbs(3)+.1))||(specfit(3)>ubs(3)-.1)
                                             specfit=[-1 -1 -1 -1];
                                         end
                                    end
                                end
                            elseif (~incfile)&&(SNR>SNRdbl)&&(specfit(2)>max(peakmx-peak1,2*fwhmmax1+fwhmmax2))
                                 broad=true;
                                 broads(i)=complex;
                                 break;
                            elseif (testbluing)&&(~incfile)&&(SNR>1.5*SNRdbl)&&(specfit(1)>SQthr)&&... % vib band may cause ambiguity!!
                                ...
                                if (fitvib)&&(SNR>2*Qthr) %fit vib band
                                    ...
                                    if (SNR>2*SNRdbl)&&(testbluing)&&(~incfile)&&(mainspecfit(4)<lbs(4)/2) % potential bluing
                                        diffspec=data(1:singlemax)-skewgaussian3(mainspecfit,wav(1:singlemax));
                                        [A,I]=max(diffspec);
                                        peaki=find(wav>=peak1-fwhm1/4,1,'first');
                                        if (A>lbs(1))&&(I<peaki)
                                            blue=true;
                                            blues(i)=complex;
                                            break
                                    ...
                                    if (finddblpeaks)&&(SNR>2*SNRdbl)&&((mainspecfit(2)>ubs(2)-.1)||(mainspecfit(4)>ubs(4)-.01))
                                        alldbl=[alldbl spec];
                                    else
    ...
    %                               if (specfit(3)-specfit(2)<peak1-2*fwhm1)&&(specfit(4)<skewmax1/2)
                                if (testbluing)&&(~incfile)&&(SNR>SNRdbl*2)&&(specfit(1)>SQthr)
                                   if (specfit(3)<peakmin+.1)||((specfit(3)<(peak1+peakmin)/2)&&(specfit(2)>fwhmdouble-.1)&&(specfit(4)<skewmax1))...
    ...
    %                                 vibbandfit=[vibpeakampl,meanvibs(2),meanvibs(3)]; %approximation
                                ...
                                if ~isempty(alldbl)   %if there is a big misfit, then prob double band
                                    ...
                                    if rms>4.8  %not 100% accurate, but identifies some more doubles. Threshold should change for noisy data!! (Check this again)
            ...
            %%%   (If very small (quenched) far-red spectra are missed, these Q spectra should be added to "allsingle"!!) 
                    ...
                    alldbl5=[];
                    if (fitvib)&&(finddblpeaks)&&(findfarreds)&&(~manualdbl)
           ...
           %%% 8. Fit double-band spectra (I'll make separate function later) %%%
                     ...
                     if (finddblpeaks)&&(~isempty(alldbl))
                        if ~manualdbl
